...
#### Calculation:
Currently, PoL is calculated daily using Location packets from the previous day.
For all station types, including M5/D1, Pulse and H1, at least one accurate Location Packet is required to determine the Location.
For H1 stations, if no Location packets are available, the PoL algorithm will rely on data from the day before.
