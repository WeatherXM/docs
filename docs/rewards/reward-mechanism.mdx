...
* Criteria that should be met for a weather station to be rewardable:
  * Data quality over qod_threshold
...
* **Data Quality**: An algorithm that evaluates the quality of weather data provided by a weather station. It generates a score that denotes the confidence about the quality of the weather data for each weather station.
 ...
 guidelines in order to consistently achieve the best Data Quality score possible. Read more about the [Data Quality algorithm](/rewards/quality-of-data).
...
* `qod_threshold`: the number denoting the cut-off threshold for Data Quality: weather stations with `qod_score < qod_threshold` are **excluded** from rewarding
...
- [Data Quality algorithm](/rewards/quality-of-data)
