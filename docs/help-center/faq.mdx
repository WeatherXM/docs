...
1. The WeatherXM Helium station has an integrated (located within the weather station) Helium-compatible LoRaWAN chip, crypto chip, and GPS while the Wi-Fi bundle has a Wi-Fi gateway, crypto chip, and GPS located within the external WG1000 gateway.
1. WeatherXM Helium station can only connect to the Helium Network while the Wi-Fi bundle can only connect to Wi-Fi.
1. The WeatherXM helium weather station has a larger solar panel which allows it to do processing standalone (in the station itself) while the Wi-Fi bundle's station has a smaller solar panel since the processing is done by the external WG1000 display and Wi-Fi gateway (located indoors) which the station communicates to over RF.
1. The WeatherXM Helium bundle does not contain any type of display to view weather data. Weather data will have to be viewed from the WeatherXM app.
...
If you have already set up your station and claimed your device via the app, please add Metamask and follow the instructions [here](/mobile-app/wallet/add-edit-wallet-address) and then follow this guide to [claim your rewards](/project/rewards-claiming).
